...
class Database:
    ...
    async def remove_ban(self, id):
        ...
        await self.col.update_one(
            {"id": id}, {"$set": {"ban_status": ban_status}}
        )

    async def ban_user(self, user_id, ban_reason="No Reason"):
        ban_status = dict(is_banned=True, ban_reason=ban_reason)
        await self.col.update_one(
            {"id": user_id}, {"$set": {"ban_status": ban_status}}
        )
    ...
    async def update_settings(self, id, settings):
        await self.grp.update_one(
            {"id": int(id)}, {"$set": {"settings": settings}}
        )
    ...
    async def use_second_shortener(self, user_id, time):
        ...
        if await self.is_user_verified(user_id):
            ...
            if time_difference > datetime.timedelta(seconds=time):
                ...
                second_time = user["second_time_verified"].astimezone(
                    ist_timezone
                )
    ...
    async def use_third_shortener(self, user_id, time):
        ...
        if await self.user_verified(user_id):
            ...
            time_difference = current_time - pastDate
            if time_difference > datetime.timedelta(seconds=time):
                pastDate = user["second_time_verified"].astimezone(
                    ist_timezone
                )
                second_time = user["third_time_verified"].astimezone(
                    ist_timezone
                )
                return second_time < pastDate
        return False
    ...
    async def get_bot_setting(self, bot_id, setting_key, default_value):
        ...
        return (
            bot[setting_key] if bot and setting_key in bot else default_value
        )
    ...
    async def update_movie_update_status(self, bot_id, enable):
        await self.update_bot_setting(
            bot_id, "MOVIE_UPDATE_NOTIFICATION", enable
        )

    # ==================== PLAYLIST / FAVORITES SYSTEM ====================

    async def add_to_playlist(self, user_id: int, file_id: str):
        """User ၏ Playlist ထဲသို့ သီချင်း ID ထည့်ခြင်း"""
        await self.col.update_one(
            {"id": int(user_id)},
            {"$addToSet": {"fav_files": file_id}},
            upsert=True,
        )

    async def remove_from_playlist(self, user_id: int, file_id: str):
        """User ၏ Playlist ထဲမှ သီချင်း ID ပြန်ဖျက်ခြင်း"""
        await self.col.update_one(
            {"id": int(user_id)}, {"$pull": {"fav_files": file_id}}
        )

    async def get_playlist(self, user_id: int):
        """User သိမ်းဆည်းထားသော Playlist သီချင်းစာရင်းကို ရယူခြင်း"""
        user = await self.col.find_one({"id": int(user_id)})
        if user and "fav_files" in user:
            return user["fav_files"]
        return []
